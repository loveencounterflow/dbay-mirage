  ...
  this.Htmlish = (function() {
    class Htmlish {
      ...
      $transpile_markdownish() {
        ...
        return (text, send) => {
          return send(md.renderInline(text));
        };
      }
...
/* obliterate remaining backslashes (exc. escaped ones) */          return send(d);
      ...
      $split_lines() {
        return (d, send) => {
          var e, i, idx, len, line, lines, reveal, text;
          if (!(d.$key === '^text')) {
            return send(d);
          }
...
/* TAINT makes `start`, `stop` invalid (but are thery still needed?) */
          ...
          return null;
          return ({text, reveal} = this._tunnel(text));
        };
      }
      ...
      parse(text, tag_catalog = null) {
        ...
        mr.push(function(text) {
          return urge('^394^', rpr(text));
        });
