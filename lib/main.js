  ...
  this.Mrg = (function() {
    class Mrg {
      ...
      _procure_infrastructure() {
        ...
        this.db(SQL`drop view   if exists _${prefix}_ws_linecounts;
...
create index ${prefix}_raw_mirror_txt on ${prefix}_raw_mirror ( txt );`);
        //.......................................................................................................
        this.db(SQL`create view ${prefix}_lines as select
    *
  ...
  order by dsk, oln, trk, pce;`);
        //.......................................................................................................
        this.db(SQL`create view _${prefix}_ws_linecounts as select distinct
  ...
  order by mirror.dsk, mirror.oln, mirror.trk, mirror.pce;`);
        //.......................................................................................................
        this.db(SQL`create view ${prefix}_paragraphs as select distinct
    raw_mirror.dsk                                as dsk,
    min( raw_mirror.oln ) over w                  as oln1,
    max( raw_mirror.oln ) over w                  as oln2,
...
limit 1;`
      ...
      refresh_datasource(cfg) {
        return this.allowing_change_on_mirror(() => {
          return this._refresh_datasource(cfg);
        });
      }
      ...
      _refresh_datasource(cfg) {
        ...
        if (force || (digest !== current_digest)) {
          ...
          this.db(() => {
            var iterator;
            iterator = GUY.fs.walk_lines(path, {
              decode: false
            });
      ...
      //---------------------------------------------------------------------------------------------------------
      append_text(cfg) {
        var counts, dsk, iterator, oln, prefix, text, trk;
        ...
        oln = (this._get_next_free_oln({dsk, trk})) - 1;
        iterator = text.split('\n');
        this.allowing_change_on_mirror(() => {
          return this._insert_lines({dsk, oln, trk, counts, iterator});
        });
        return counts;
      }

      //---------------------------------------------------------------------------------------------------------
      _get_next_free_oln(cfg) {
        return this.db.single_value(this.sql.next_free_oln, cfg);
      }

      _get_next_free_par(cfg) {
        return this.db.single_value(this.sql.next_free_par, cfg);
      }

      //---------------------------------------------------------------------------------------------------------
      _insert_lines(cfg) {
        var counts, dsk, error, insert_line_into_mirror, insert_line_into_raw_mirror, iterator, line, oln, par, prefix, row, trim_line_ends, trk, txt, within_par;
        ({dsk, oln, trk, counts, iterator} = cfg);
        ({prefix} = this.cfg);
        ...
        par = (this._get_next_free_par({dsk, trk})) - 1;
        within_par = false;
        trim_line_ends = this.constructor.C.trim_line_ends;
//.......................................................................................................
        for (line of iterator) {
          oln++;
          ...
          try {
            ...
            if (error.code === 'SQLITE_CONSTRAINT_PRIMARYKEY') {
              throw new this.db.E.DBay_internal_error('^dbay-mirage@1^', `not unique in mrg_mirror or mrg_raw_mirror: ${rpr(row)}`);
            }
            throw error;
          }
        }
      ...
      walk_line_rows(cfg) {
        ...
        this.db.setv('trk', trk != null ? trk : 1);
      ...
      walk_par_rows(cfg) {
        ...
        ({prefix} = this.cfg);
        this.db.setv('dsk', dsk);
        return this.db.alt(SQL`select * from ${prefix}_paragraphs;`);
      }
