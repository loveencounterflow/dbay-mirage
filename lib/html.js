(function() {
  ...
  this.Htmlish = (function() {
    class Htmlish {
      ...
      //---------------------------------------------------------------------------------------------------------
      parse(text) {
        var $complain_about_bareachrs, $handle_stack_close, $handle_stack_open, $parse_ncrs, $reinstate_text, $treat_xws_in_closing_tags, $treat_xws_in_opening_tags, R, mr, reveal, stack, tokens, xncr_matcher, xncr_splitter;
        ...
        mr.push($parse_ncrs = (d, send) => {
          var e, i, is_entity, len, match, part, parts, start, stop;
          if (!(d.$key === '^text')) {
            return send(d);
          }
