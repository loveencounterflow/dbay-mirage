...
drop  table if exists ${prefix}_html_atrids;`);
    ...
    escape_ltamp        boolean not null default false );`);
        //-------------------------------------------------------------------------------------------------------
        db(SQL`create table ${prefix}_html_fences (
  ...
  check ( ( open is     null and close is     null and either is not  null ) or
        ...
        db(SQL`create table ${prefix}_html_fence_matches (
    dsk                 text    not null,
...
create index ${prefix}_html_mirror_tag_idx on ${prefix}_html_mirror ( tag );`);
  ...
  order by t.dsk, t.oln, t.trk, t.pce;`);
      ...
      _procure_infradata() {
        /* TAINT skip if tables found */
        var db, html_data, insert_fence, insert_syntax, insert_tag, prefix;
        ({prefix} = this.cfg);
        ({db} = this.mrg);
        ({insert_syntax, insert_fence, insert_tag} = this.statements);
        ...
        db(() => {
          ...
          try {
            ref = html_data.fences;
            results = [];
            for (i = 0, len = ref.length; i < len; i++) {
              d = ref[i];
              ({name, environment, syntax, open, close, either} = d);
              this._fence_catalog[name] = {name, environment, syntax, open, close, either};
              ...
              results.push(insert_fence.run({name, environment, syntax, open, close, either}));
            }
            return results;
          } catch (error1) {
            error = error1;
        ...
        this._fence_catalog = freeze(this._fence_catalog);
        return null;
      }
...
insert into ${prefix}_html_mirror ( dsk, oln, col, trk, pce, typ, tag, syntax, atrid, txt )
  ...
  returning *;`),
          ...
          insert_fence: db.prepare_insert({
            into: `${prefix}_html_fences`,
            returning: null
          }),
          insert_fence_matches: db.prepare_insert({
            into: `${prefix}_html_fence_matches`,
            returning: null
          })
        });
        //.......................................................................................................
      ...
      parse_dsk(cfg) {
        ...
        ({dsk} = cfg);
        this._collect_fence_matches({dsk});
        //.......................................................................................................
        this.mrg.db.with_transaction(() => {
          var _, atrs, col, d, i, len, oln, oln1, par, pce, ref, ref1, ref2, ref3, ref4, results, syntax, tokens, trk, txt, wslc, y;
...
//...................................................................................................
            for (i = 0, len = tokens.length; i < len; i++) {
              d = tokens[i];
              oln = oln1 + ((ref1 = d.delta_lnr) != null ? ref1 : 0);
              col = (ref2 = d.col) != null ? ref2 : 0;
              syntax = (ref3 = d.syntax) != null ? ref3 : 'html';
              switch (d.$key) {
                case '<tag':
            ...
            results.push((function() {
              var j, ref5, results1;
              results1 = [];
              for (_ = j = 1, ref5 = wslc + 1; (1 <= ref5 ? j <= ref5 : j >= ref5); _ = 1 <= ref5 ? ++j : --j) {
      ...
      * _walk_pattern_matches(text, pattern) {
      ...
      _get_zone_candidates(text) {
        var R, close, d, either, fence, hit, open, ref, ref1, ref2, ref3, syntax;
        R = [];
        ref = this._fence_catalog;
        for (fence in ref) {
          d = ref[fence];
      ...
      _collect_fence_matches(cfg) {
        ...
        var cache, current_fence, d, dsk, fence, fnr, i, j, len, len1, oln, pce, ref, ref1, role, row, start, stop, trk, txt, y;
        ({dsk} = cfg);
        cache = [];
        current_fence = null;
        ...
        for (y of ref) {
          ...
          for (i = 0, len = ref1.length; i < len; i++) {
            d = ref1[i];
            ({role, fence, start, stop} = d);
            fence = this._fence_catalog[fence];
            if (current_fence != null) {
              if (role === 'either') {
                if (current_fence === fence.name) {
                  current_fence = null;
                  role = 'close';
                } else {
                  continue;
                ...
                if (!((fence.name === current_fence) && (role === 'close'))) {
                  continue;
                }
                current_fence = null;
              }
        ...
        for (j = 0, len1 = cache.length; j < len1; j++) {
          row = cache[j];
          //.......................................................................................................
          this.mrg.db(this.statements.insert_fence_matches, row);
        }
        cache.length = 0;
        //.......................................................................................................
        return null;
