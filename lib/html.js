  ...
  order by tid;`);
      ...
      _compile_statements() {
        ...
        GUY.props.hide(this, 'statements', {
          ...
          insert_content: db.prepare(SQL`with v1 as ( select coalesce( max( tid ), 0 ) + 1 as tid from ${prefix}_html_mirror where dsk = $dsk )
insert into ${prefix}_html_mirror ( dsk, tid, typ, tag, atrid, text )
  ...
  returning *;`),
      ...
      _append_tag(dsk, typ, tag, atrs = null, text = null) {
        ...
        return this.mrg.db.first_row(this.statements.insert_content, {dsk, typ, tag, atrid, text});
      }
      ...
      //---------------------------------------------------------------------------------------------------------
      parse_dsk(cfg) {
        var atrs, d, dsk, i, len, par, ref1, rwn1, rwn2, tokens, txt, y;
        ...
        for (y of ref1) {
          ...
          for (i = 0, len = tokens.length; i < len; i++) {
            ...
            switch (d.$key) {
              case '<tag':
                this._append_tag(dsk, '<', d.name, d.atrs);
                break;
              case '>tag':
                this._append_tag(dsk, '>', d.name, d.atrs);
                break;
              case '^tag':
                ...
                break;
              case '^text':
                this._append_tag(dsk, 't', null, null, d.text);
                break;
              case '^comment':
                this._append_tag(dsk, 'r', null, null, d.text.replace(/^<!--\s*(.*?)\s*-->$/, '$1'));
                break;
              case '^error':
                atrs = {
                  start: d.start,
                ...
                this._append_tag(dsk, 'e', null, atrs, `${d.message}: ${rpr(d.text)}`);
                break;
              default:
                ...
                this._append_tag(dsk, 'e', null, atrs, `unhandled token: ${rpr(d)}`);
            }
          }
        }
        return null;
