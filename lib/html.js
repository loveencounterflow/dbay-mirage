...
insert into ${prefix}_html_mirror ( dsk, oln, trk, pce, typ, tag, atrid, txt )
  ...
  returning *;`),
      ...
      _append_tag(dsk, oln, trk, typ, tag, atrs = null, text = null) {
        ...
        return this.mrg.db.first_row(this.statements.insert_content, {
          dsk,
          oln,
          trk,
          typ,
      ...
      parse_dsk(cfg) {
        ...
        this.mrg.db.with_transaction(() => {
          var atrs, d, oln, ref1, results, tokens, trk, txt, y;
          ref1 = this.mrg.get_par_rows({dsk});
          results = [];
          for (y of ref1) {
            ({oln, trk, txt} = y);
            if (txt === ''/* NOTE we assume `@constructor.C.trim_line_ends == true` */) {
              this._append_tag(dsk, oln, trk, 'b', null, null, '');
              continue;
            }
            ...
            results.push((function() {
              ...
              for (i = 0, len = tokens.length; i < len; i++) {
                ...
                switch (d.$key) {
                  case '<tag':
                    results1.push(this._append_tag(dsk, oln, trk, '<', d.name, d.atrs));
                    break;
                  case '>tag':
                    results1.push(this._append_tag(dsk, oln, trk, '>', d.name, d.atrs));
                    break;
                  case '^tag':
                    ...
                    break;
                  case '^text':
                    results1.push(this._append_tag(dsk, oln, trk, 't', null, null, d.text));
                    break;
                  case '^comment':
                    results1.push(this._append_tag(dsk, oln, trk, 'r', null, null, d.text.replace(/^<!--\s*(.*?)\s*-->$/, '$1')));
                    break;
                  case '^error':
                    atrs = {
                      start: d.start,
                    ...
                    results1.push(this._append_tag(dsk, oln, trk, 'e', null, atrs, `${d.message}: ${rpr(d.text)}`));
                    break;
                  default:
                    ...
                    results1.push(this._append_tag(dsk, oln, trk, 'e', null, atrs, `unhandled token: ${rpr(d)}`));
                }
              }
              return results1;
            }).call(this));
